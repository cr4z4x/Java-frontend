...
# OpenAI router
router = APIRouter()
openai.api_key = "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"
...
def process_text(request_data: AIRequest, db: Session = Depends(get_db)):
    try:
        ...
        history_entry = History(text=request_data.text, response=response.choices[0].text, type=request_data.type, id=request_data.id)
